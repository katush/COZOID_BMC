...
\textcolor{blue}{Studying the patterns of protein-protein interactions (PPIs) is fundamental for understanding the structure and function of biological complexes. 
The exploration of the vast space of possible mutual configurations of interacting proteins and their contact zones is very time consuming and requires the domain expert knowledge.} 
\parttitle{Results}
...
The first visual abstraction method, the Matrix view, is based on customized interactive heat maps and provides the users with an overview of all possible residue-residue contacts in PPI configurations and their interactive filtering. 
...
These representations solve the problem of high overlaps of the configurations. 
Using these views, the structural alignment of the best models can be visually confirmed as well.
\parttitle{Conclusions}
\textcolor{red}{We developed a system for exploration of large sets of protein-protein complexes in fast and intuitive way. The usefulness of our system was verified on several docking structures covering all three types of PPIs, i.e, coiled-coil, pocket-string, and surface-surface interactions. Our case studies prove, that our tool helps to analyze and filter protein-protein complexes in a fraction of time spent when using the previously available techniques.}
\end{abstract}
...
\textcolor{blue}{It is estimated that about 800 complexes exist in just one yeast cell~\cite{Gavin}. }
...
\textcolor{red}{However, these algorithms produce a large number of possible configurations, which need to be explored to identify the biochemically most relevant ones}.
...
\textcolor{red}{Abstracted representations were already successfully applied to protein-ligand docking.
LigPlot+~\cite{laskowski2011ligplot+} is an interactive system for generation of 2D protein-ligand interaction diagrams. 
In this tool a flattened 2D representation of ligand is placed at the center of the diagram and the interacting protein residues are placed around it at corresponding positions.  Superimposition ca be used to compare multiple diagrams, such as one multiple ligands binding to the same protein, or same ligand binding to homologous proteins. 
However, this approach is suitable only for small ligand molecules and does not scale well to PPIs.}
...
\textcolor{red}{In this paper we present a systemic tool, COZOID (COntact ZOne IDentifier), comprised of a set of methods for the visualization, comparison, and selection of numerous docking configurations.
...
Our tools is integrated into CAVER Analyst~\cite{kozlikova2014caver} software tool for analysis of biomolecules and thus contains also other relevant features, such as different molecular visualization modes, measurement tools, etc.  
...
\textcolor{red}{The decision is based on various pieces of knowledge about the geometric arrangement of the configuration as well as other aspects, such as the physico-chemical properties of the amino acids present in the contact zone of the given configuration.
...
\textcolor{blue}{The interaction is performed by clicking on individual cells. 
...
\textcolor{red}{From the color of the matrix cells the user can immediately estimate} the number of configurations in which these pairs are present as well.
...
\textcolor{red}{As the change in position of the exploded proteins can cause disorientation, the pairing information between corresponding reference proteins (aligned) and paired proteins is initially indicated as a partially transparent tube that connects the centers of their contact zones.
The radius of the tube is modulated -- it is smaller in the middle of the tube to reduce the visual clutter.
Once the user gains the overview of the spatial arrangement of the proteins, the tube can be off.}
...
\textcolor{red}{All presented techniques and their integration into the proposed COZOID tool were designed in a tight collaboration between the domain experts from the field of visualization and the field of functional proteomics.
...
The current main research focus of our propteomic experts is on the structure maintenance of chromosome (SMC) complexes~\cite{Palecek2015}. }
...
To demonstrate the usability of our proposed techniques, \textcolor{red}{we selected} representatives of three basic types of PPI patterns, occurring not only in the SMC complexes but also in other multiprotein assemblies. 
...
The surface-surface interaction was tested on NSE1 and NSE3 proteins of SMC complex. 
...
The crystal structure of the human NSE1-NSE3 dimer was already examined in detail and the resulting configuration is already published in the PDBsum database under the PDB identifier \textcolor{blue}{3NW0}. 
...
To restrict the set of possible docking configurations, \textcolor{red}{we selected} the web version of the HADDOCK tool and a pair of interacting amino acids, i.e., methionine with ID 23 from the reference protein and leucine with ID 97 from the paired protein.
...
The matrix identified configurations containing the pairs of interacting amino acids with the interaction \textcolor{blue}{distances smaller than 4 \AA.}
...
In the next step, \textcolor{red}{we} switched to the \CoZoListView and compared the list of amino acids of the \textcolor{blue}{3NW0} crystal structure with the lists of all three selected configurations.
Figure~\ref{fig:case3} shows the comparison between the \textcolor{blue}{3NW0} structure and three selected HADDOCK configurations.
...
Even from the given part of the \CoZoListView, the similarities and differences between the crystal \textcolor{blue}{3NW0} (in the leftmost list) and the three selected HADDOCK configurations on the level of individual amino acids are clearly visible.
In addition, the pairs of the interacting amino acids identical to the \textcolor{blue}{3NW0} crystal structure are highlighted (red lines in Figure~\ref{fig:case3}). 
...
Finally, the \textcolor{blue}{3NW0} crystal and three selected configurations were explored using the 3D representations with the aim to explore the constitution, the mutual distances, and the properties of the contact zones in detail.
In \textcolor{blue}{3NW0}, the first interacting protein was selected as the reference protein and all three configurations were aligned with respect to the paired proteins.
...
Moreover, if \textcolor{red}{we point} the camera towards the aligned reference proteins, the differences between the positions of the contact zones in the reference proteins can be observed as well.
%If rotating the scene, the user can easily lose the information about the direction of the shift between the proteins.
The \ExpView representation gave \textcolor{red}{us} the information about the mutual positioning of the individual configurations with respect to the positions of the contact zones.
...
\textcolor{red}{By animating the opening of the protein complex, we were able to look inside the contact zone.
The enhancements of the \OpBook, i.e., labeling the surface of the contact zones with the names of the corresponding amino acids, and coloring them according to different properties, were highly beneficial for and exploration of the physico-chemical and geometric properties of the individual amino acids.}% appreciated by the domain expert.
...
These amino acids might be selected based on the COZOID analysis of the \textcolor{blue}{3NW0} crystal -- using the \MatView or the \ExpView (searching for the most central and the closest amino acids).

Altogether, \textcolor{red}{COZOID} helped to quickly select the best docking configuration using several visualization approaches. 
First, the \MatView allowed \textcolor{red}{us} to pick models containing a particular pair of interacting amino acids. 
Next, with the \CoZoList, \textcolor{red}{we} sorted these models based on the similarity of their contact zones with the original crystal structure. 
Using the 3D \ExpView, the best model was determined and confirmed. 
While the Exploded view is already available in some of the current 3D visualization tools, the power of its combination with our other proposed approaches lies in the speed, \textcolor{red}{user friendly design}, and high interactivity of the selection mechanism. 
...
For the second type of interaction, \textcolor{red}{we} picked the SMC3 coiled-coil arm of the SMC complex~\cite{Gligoris}.
...
Using the \MatView, \textcolor{red}{we} set the interaction distance threshold between 3 and 5~\AA~and selected the methionine 186 and isoleucine 1030 as the initial pair of interacting amino acids (Figure~\ref{fig:coiled_haddock_mat}). 
...
Afterwards, \textcolor{red}{we} selected the first amino acid (A172) within the respective helices and visually compared the positions in the 3D view.
...
The next selection provided \textcolor{red}{us} with 14 models with correct orientation (see Figure~\ref{fig:selection2SMC3PyDock}), which was a significant improvement in comparison with the HADDOCK results.

In the final step, \textcolor{red}{we} compared the \CoZoLists of the selected models with the original crystal structure (4UX3). 
...
In this case, \textcolor{red}{we} selected the interaction present in the crystal structure of the MukE-MukF complex -- proteins involved in chromosome partitioning in Escherichia coli. 
...
This time, \textcolor{red}{we} selected the valine 200 and arginine 300 as the pair of amino acids for the docking restraint. 
...
Then, \textcolor{red}{we} repeated the \MatView filtering using the second tightest amino acid contact in the crystal -- the tyrosine 110 and arginine 302. 
...
Here, \textcolor{red}{we} took the advantage of our test set-up (using the tightest contacts between the interacting amino acids) and altered the interaction distance parameter in the \MatView for the selection procedure. 
...
\textcolor{red}{We tested these methods on real datasets for SMC complexes and we demonstrated the usability on three studies, covering the most common interaction patterns.
It was confirmed, that our proposed solution provides the proteomic experts with information, which was very hard or even impossible to get using the previously available methods.
The studies confirmed, that using our solution, the exploration process can lead to a satisfying conclusion about the biochemical relevance of individual configurations much faster.}
...
\textcolor{blue}{In the future, we plan to focus on the extension of our proposed techniques to cases where the user has no a priori knowledge about the protein complex.
We also plan to extend the methods to complexes consisting of more than two interacting proteins. 
Additionally, the COZOID tool is already used for training students of proteomics to analyze and understand the crystal structures as well as the computed models.
...
We acknowledge members of Palecek lab for their participation in COZOID testing. 
...
\begin{figure}[!h]
  ...
  \caption{\csentence{Coiled-Coil Interaction -- best fitting PyDock configuration.} The \ExpView showing the contact zone of the best fitting PyDock model (orange) and the 4UX3 crystal (blue). On top the overlapping contact zones on the reference protein are shown. The bottom part of the image depicts the paired proteins.}
