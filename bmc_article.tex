...
Here it is important to understand that the reference protein shown in the figure (the biggest one) actually {\color{red} consists of (represents)} three overlapping aligned proteins, each coming from one configuration.
...
To ensure that the paired proteins in the exploded view will not collide with each other, {\color{red} (our algorithm calculates the spatial position of paired proteins based on their bounding boxes) we employ a simple iterative force-directed placement algorithm, where paired proteins repulse each other \cite{fruchterman1991graph}}.
...
We display the connection between corresponding reference proteins (aligned) and paired proteins as a {\color{red} partially transparent} tube connecting the centers of their contact zones.
The radius of the tube is modulated -- it is smaller in the middle of the tube to reduce visual clutter.
{\color{red}(The opacity of the tube is modulated in screen space, depending on the distance to the tube centerline)}.
...
\begin{figure}[h!]
  ...
  \includegraphics[width=\columnwidth]{images/figure8.pdf}
    \caption{\csentence{Sorting of the \CoZoListView.} Sorting of the \CoZoListView according to different properties of amino acids -- (a) hydrophobicity, (b) mutual distance, (c) frequency of occurrence.}
  \label{fig:sorting}
\end{figure}
...
\begin{figure}[!h]
  ...
  \caption{\csentence{Best fit PyDock.} \OpBook showing contact zone of  the best fitting PyDock model (orange) and 4UX3 crystal (blue).}
